...
verticalDist = [0,0.182,0.097] # TF x,y,z unit:m
...
exResult = np.array((0,2.8,0,0,0,0)).reshape(6,1) #strafe, drive, updown, tiltFR, tiltLR, yaw

result = movementListener(exResult)

for counter, x in enumerate(result):
    if x < 0:
        result[counter,0] *= 1.65
...
print(max)
for x in result:
    result[counter,0] = result[counter,0] / max
